 ...
 * Displays an image
 **/
void display_image(CImg<unsigned char> &image, char *str){
	CImgDisplay display(image, str);
	display.wait();
}
 ...
 * @r : offset table size
 **/
int perfect_hashing(CImg<unsigned char> &image, CImg<unsigned char> &hash,
					CImg<unsigned char> &offsets, int &n, int &m, int &u, int &r)
{
...
int main()
{
	...
	//Loads main image
	CImg<unsigned char> image("sample.bmp");
	...
	while (perfect_hashing(image, hash, offsets, n, m, u, r) == 0)
	{
		cout << endl;
		r = ceil(r*1.2);
		while (pgcd(r, m) != 1)
